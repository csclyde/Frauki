...
EffectsController.prototype.UpdateEffects = function() {
	...
	this.fluff.forEachAlive(function(particle) {
		particle.body.velocity.x += Math.floor((Math.random() * 10) - 5);
		particle.body.velocity.y += Math.floor((Math.random() * 10) - 5);

		if(particle.body.velocity.x > MAX_FLUFF_SPEED * 2)
			particle.body.velocity.x = MAX_FLUFF_SPEED * 2;
		if(particle.body.velocity.x < -MAX_FLUFF_SPEED * 2)
			particle.body.velocity.x = -MAX_FLUFF_SPEED * 2;
