...
GameState.CreateUI = function() {
    ...
    this.menuText = [];
    for(var i = 0; i < 6; i++) {
        var text = game.add.bitmapText(pixel.width / 2, 240 + i * 20, 'bubble','', 18);
        this.Menu.add(text);
    ...
    this.soundSliderFrame = game.add.image(400, 229, 'UI', 'Settings0000', this.settingsMenu);
    this.soundPips = [];

    this.musicSetting = game.add.bitmapText(400, 249, 'bubble','ON', 18);
    this.settingsMenu.add(this.musicSetting);
    this.sfxSetting = game.add.bitmapText(400, 269, 'bubble','OFF', 18);
    this.settingsMenu.add(this.sfxSetting);


    for(var i = 0; i < 8; i++) {
        var soundPip = game.add.image(407 + (i * 7), 232, 'UI', 'Settings0001', this.settingsMenu);
